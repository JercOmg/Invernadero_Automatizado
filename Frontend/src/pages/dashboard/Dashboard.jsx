...
const Dashboard = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 space-y-10 animate-fade-in">
      ...
      <header className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-8 md:p-10 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.03)] flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="absolute top-0 right-0 -mt-8 -mr-8 w-64 h-64 bg-emerald-50/30 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 -mb-8 -ml-8 w-64 h-64 bg-slate-100/20 rounded-full blur-3xl pointer-events-none"></div>
        
        <div className="space-y-3 z-10">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-slate-900">
            {t('dashboard.welcome')}, <span className="text-emerald-600 font-black">{user?.nombre} {user?.apellido}</span>
          </h1>
        ...
        <div className="z-10 shrink-0">
          <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 border border-emerald-200/60 px-4 py-2 rounded-lg text-xs md:text-sm font-bold uppercase tracking-wider shadow-xs">
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {modules.map((mod) => (
            <div 
              key={mod.id}
              className="group relative overflow-hidden rounded-xl border border-slate-200/80 bg-white p-6 shadow-[0_4px_20px_-2px_rgba(15,23,42,0.02)] transition-all duration-300 hover:-translate-y-1 hover:border-emerald-500/30 hover:shadow-[0_12px_30px_rgba(16,185,129,0.06)] flex flex-col justify-between min-h-[220px]"
            >
              <div>
                {/* Icono con resplandor en modo claro */}
                <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-200/60 flex items-center justify-center text-2xl mb-5 group-hover:scale-110 group-hover:border-emerald-500/20 group-hover:bg-emerald-50 transition-all duration-300 shadow-2xs">
                  <span className="drop-shadow-[0_2px_4px_rgba(0,0,0,0.05)]">{mod.icon}</span>
                </div>

                {/* Título */}
                <h3 className="text-base font-bold text-slate-800 group-hover:text-emerald-600 transition-colors duration-300">
                  {t(mod.titleKey)}
                </h3>

                {/* Descripción */}
                <p className="text-xs text-slate-500 mt-2 line-clamp-3 font-medium leading-relaxed">
                  {t(mod.descKey)}
                </p>
              </div>
